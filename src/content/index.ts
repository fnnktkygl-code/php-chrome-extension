...
class QuickPasteMenu {
  ...
  private filteredClips: QuickClip[] = [];
  private isFr = true;
  private isDark = true;
  private searchQuery = '';
  ...
  public async open(): Promise<void> {
    ...
    const searchInput = searchBar.querySelector('#php-qp-search-input') as HTMLInputElement;
    const countEl = searchBar.querySelector('#php-qp-count') as HTMLSpanElement;
  ...
  private renderList(list: HTMLElement, borderCard: string, textMain: string, textSub: string): void {
    list.innerHTML = '';
    if (this.filteredClips.length === 0) {
      list.innerHTML = `
